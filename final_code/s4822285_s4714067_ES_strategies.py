...
def s4822285_s4714067_ES_classic(problem: ioh.problem.BBOB, budget: int, seed: int, cfg: ESConfig) -> RunSummary:
    t0 = walltime()
...
def _run_config(cfg: ESConfig, budget: int, runs: int, base_seed: int, data_root: Path, instance: int) -> None:
    fid = DEFAULT_FID
    dim = DEFAULT_DIM
    algo_name = f"s4822285_s4714067_ES_{cfg.strategy_label}"
    ...
    for r in range(runs):
        seed = base_seed + r
        rs = s4822285_s4714067_ES_classic(problem, budget=budget, seed=seed, cfg=cfg)
        rs.run_index = r
