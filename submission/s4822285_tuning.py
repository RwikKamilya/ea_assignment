...
Hyper-parameter tuning for the GA (Part 1) under the assignment constraint:
  - Total tuning budget ≤ 100,000 function evaluations
  - Find ONE configuration that works for both problems

We do a 2-stage random search:
  Stage 1 (coarse): many cheap trials at eval_budget=1000
  Stage 2 (refine): re-evaluate the top-2 configs at the full eval_budget=5000

Scoring:
  - For each config we compute the median final best across seeds for each task.
  - We then z-normalize medians across configs (per task) and sum the z-scores.

Outputs:
  - ga_best_params.json (loaded automatically by s4822285_GA.py)
...
from s4822285_GA import GAParams, s4822285_GA
...
def main() -> None:
    ...
    # ----- Rank configs by z-normalized sum across tasks -----
    ...
    if not top_idx:
        # Not enough remaining budget: just pick best from stage 1
    ...
    # Stage 2 seeds should be different from Stage 1 seeds.
    # Use a slice after the Stage 1 seed usage (not a hard-coded 100_000 offset).
    ...
    # z-normalize across candidates (per task) and pick best sum-z
    arr = np.asarray(stage2_details, dtype=float)  # [n_cand, n_tasks]
    z = np.zeros_like(arr)
